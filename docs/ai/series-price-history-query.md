...
- **Bounds the work, not just the response.** The result is capped at `MAX_PRICE_HISTORY_POINTS = 1000` candles, keeping the **most recent** buckets (a chart reads the latest window). Crucially, the scan runs **newest→oldest** and stops as soon as it would open a 1001st bucket, so an open-ended (`None`/`None`) request on a long-lived, heavily-traded market processes only the points in its newest 1000 buckets rather than every trade ever recorded. The index is in execution order, which is chronological (trades execute sequentially, so a higher `event_id` never carries an earlier timestamp), so reversing it visits buckets in strictly non-increasing start time: once the cap is full, the next not-yet-seen bucket is strictly older than every kept one and ends the scan, and the oldest kept bucket is always fully aggregated first. Each point folds into a `BTreeMap<bucket_start, candle>`, which yields the candles time-ordered with no separate sort. (A degenerate `end_time` far in the past still skips the newer points above the window before terminating; in normal use `end_time` is `None` or near-now.)
