use candid::{CandidType, Nat};
use serde::{Deserialize, Serialize};

/// Mechanism for ensuring idempotency in ledger payments.
#[derive(CandidType, Serialize, Deserialize, Clone, Debug)]
pub enum PaymentIdempotency {
    /// Uses the ICRC `created_at_time` field to prevent duplicate transfers.
    IcrcCreatedAtTimeNs(u64),
}
impl PaymentIdempotency {
    /// Converts the idempotency key to an optional timestamp in nanoseconds.
    pub fn to_created_at_time_ns(&self) -> Option<u64> {
        match self {
            PaymentIdempotency::IcrcCreatedAtTimeNs(time) => Some(*time),
        }
    }
}
impl From<u64> for PaymentIdempotency {
    fn from(value: u64) -> Self {
        PaymentIdempotency::IcrcCreatedAtTimeNs(value)
    }
}

/// Proof of successful payment on a ledger.
#[derive(CandidType, Serialize, Deserialize, Clone, Debug)]
pub enum PaymentReceipt {
    /// The block index in which the transfer was recorded.
    IcrcBlockIndex(Nat),
}
impl PaymentReceipt {
    pub fn block_index(&self) -> Nat {
        match self {
            PaymentReceipt::IcrcBlockIndex(index) => index.clone(),
        }
    }
}
impl From<Nat> for PaymentReceipt {
    fn from(value: Nat) -> Self {
        PaymentReceipt::IcrcBlockIndex(value)
    }
}
