...
mod tests {
    ...
    #[test]
    fn test_equity_and_worth_calculation() {
        ...
        // 1 ICP * $10 * 0.9 = $9
        let equity = state.calculate_equity_usd(&configs, &metrics);
        assert_eq!(equity, 9_000_000);

        state.reserved_margin_usd = 5_000_000; // $5
        let available = state.get_available_equity_usd(&configs, &metrics);
        assert_eq!(available, 4_000_000);
    }
}
